...
try {
    ...
    if (!empty($_SESSION['user_id64'])) {
        ...
        if ($db) {
            ...
            $lobbyMap = !empty($_POST['lobby_map'])
                ? htmlentities($_POST['lobby_map'])
                : NULL;

            if (!empty($lobbyID) && !empty($lobbyMap)) {
                $sqlResult = $db->q(
                ...
                if (!empty($sqlResult) && $sqlResult[0]['lobby_leader'] == $_SESSION['user_id64']) {
                    ...
                    $sqlResult = $db->q(
                        'UPDATE `lobby_list` SET `lobby_map` = ? WHERE `lobby_id` = ?;',
                        'si',
                        $lobbyMap, $lobbyID
                    );
                    ...
                    if (!empty($sqlResult)) {
                        //RETURN LOBBY ID
                        $json['result'] = 'Lobby ' . $db->last_index() . ' updated!';
                    } else {
