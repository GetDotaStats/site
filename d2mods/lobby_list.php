        ...
        if (!empty($lobbyListActive)) {
            ...
            foreach ($lobbyListActive as $key => $value) {
                $lobbyName = !empty($value['lobby_name'])
                    ? htmlentities(urldecode($value['lobby_name']))
                    : 'Custom Game #' . $value['lobby_id'];

                echo '<tr>
                        <td class="vert-align"><a class="nav-clickable" href="#d2mods__lobby?id=' . $value['lobby_id'] . '">' . $lobbyName . '</a></td>
                        <td class="vert-align">' . htmlentities(urldecode($value['lobby_leader_name'])) . ' <a target="_blank" href="#d2mods__search?user=' . $value['lobby_leader'] . '"><span class="glyphicon glyphicon-search"></span></a></td>
