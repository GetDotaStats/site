                    ...
                    $sqlResult = $db->q(
                        "INSERT INTO `cron_hs_temp`
                            SELECT
                                    ...
                                    sh1.`highscore_value`,
                                    sh1.`date_recorded`
                                FROM `stat_highscore` sh1
                                JOIN (
                                    SELECT
                                        `user_id32`,
                                        $mgObjective1(`highscore_value`) as `adjusted_value`
                                    FROM `stat_highscore`
                        ...
                        'ssss',
                        array($minigameID, $leaderboard, $minigameID, $leaderboard)
                    );
            ...
            if (!empty($leaderboards)) {
                $sqlResult = $db->q(
                    "CREATE TABLE IF NOT EXISTS `cron_hs_mod_temp` (
                      ...
                      `highscoreID` bigint(255) NOT NULL,
