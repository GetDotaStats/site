...
try {
    ...
    foreach ($activeMods as $key => $value) {
        try {
            ...
            echo "Matches: {$customGameValues}<br />";
                ...
                foreach ($schemaFields as $key2 => $value2) {
                    try {
                        ...
                        if (($max <= ($firstGroupMaxCategories + 10)) || ($quart75 < $firstGroupMaxCategories)) {
                            ...
                            echo "<ul><li>Third quartile not above {$firstGroupMaxCategories} or maximum value not greater than {($firstGroupMaxCategories + 10)}!</li></ul>";
                            echo '</ul>';
                            continue;
                        }
                        ...
                        $db->q(
                            'DELETE FROM `cache_custom_game_values_temp3` WHERE `modID` = ? AND `fieldOrder` = ?;',
                            'ii',
                            array($modID, $value2['fieldOrder'])
                        );
                        ...
                        $db->q(
                            'DELETE FROM `cache_custom_game_values_temp2` WHERE `fieldValue` < ?;',
                            'i',
                            array($firstGroupLimit)
                        );
                ...
                $customGameValueCombos = $db->q(
                    'INSERT INTO `cache_custom_game_values_temp0`
                        SELECT
                            ...
                            s2mc.`fieldValue`,
                            COUNT(*) AS numGames
                        FROM `cache_custom_game_values_temp3` s2mc
            ...
            try {
                $serviceName = 's2_cron_cmgv_' . $modID;
                ...
                service_report($serviceName, $runTime, $customGameValues, $customGameValueCombos, NULL, TRUE);
                ...
                if (!empty($oldServiceReport)) {
                    ...
                    if (
                        (empty($oldServiceReport['performance_index1']) && !empty($customGameValues)) ||
                        (empty($oldServiceReport['performance_index2']) && !empty($customGameValueCombos))
                    ) {
                        throw new Exception("Mod `{$modName}` has game values to use since the last report!");
                    }
                    ...
                    if (
                        ...
                        throw new Exception("Mod `{$modName}` had game values in the last report, but now does not!");
                    }
                    ...
                    if ($customGameValues > ($oldServiceReport['performance_index1'] * 1.5)) {
                        throw new Exception("Major increase (>50%) in performance index #1 for `{$modName}`! {$oldServiceReport['performance_index1']} game values to {$customGameValues} game values");
                    }

                    //Check if the performance_index2 increased majorly
                    if ($customGameValueCombos > ($oldServiceReport['performance_index2'] * 1.5)) {
                        throw new Exception("Major increase (>50%) in performance index #2 for `{$modName}`! {$oldServiceReport['performance_index2']} game value combos to {$customGameValueCombos} game value combos");
                    }
                }
    ...
    $db->q('RENAME TABLE `cache_custom_game_values` TO `cache_custom_game_values_old`, `cache_custom_game_values_temp0` TO `cache_custom_game_values`;');
    ...
    try {
        ...
        service_report($serviceName, $totalRunTime, $totalCustomGameValues, $totalCustomGameValueCombos, NULL, FALSE);
        ...
        //Check if the performance_index1 increased majorly
        if ($totalCustomGameValues > ($oldServiceReport['performance_index1'] * 1.1)) {
            throw new Exception("Major increase (>10%) in performance index #1! {$oldServiceReport['performance_index1']} game values to {$totalCustomGameValues} game values");
        }

        //Check if the performance_index2 increased majorly
        if ($totalCustomGameValueCombos > ($oldServiceReport['performance_index2'] * 1.1)) {
            throw new Exception("Major increase (>10%) in performance index #2! {$oldServiceReport['performance_index2']} game value combos to {$totalCustomGameValueCombos} game value combos");
        }
