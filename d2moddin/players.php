    ...
    if ($db) {
...

        {
            ...
            $production_stats = simple_cached_query('d2moddin_production_players_last4days',
                'SELECT HOUR(`date_recorded`) as hour, DAY(`date_recorded`) as day, MONTH(`date_recorded`) as month, YEAR(`date_recorded`) as year, SUM(`players_online`) as players_online, SUM(`players_playing`) as players_playing FROM `stats_production_players` WHERE `date_recorded` >= now() - INTERVAL 4 DAY GROUP BY 4,3,2,1 ORDER BY 4 DESC,3 DESC,2 DESC,1 DESC;',
                60);
            ...
            foreach ($production_stats as $key => $value) {
                ...
                $super_array[] = array('c' => array(array('v' => $date), array('v' => $value['players_online']), array('v' => $value['players_playing'])));
            }
...

        {
            ...
            $production_stats = simple_cached_query('d2moddin_production_players_alltime',
                'SELECT DAY(`date_recorded`) as day, MONTH(`date_recorded`) as month, YEAR(`date_recorded`) as year, SUM(`players_online`) as players_online, SUM(`players_playing`) as players_playing FROM `stats_production_players` GROUP BY 3,2,1 ORDER BY 3 DESC,2 DESC,1 DESC;',
                60);
