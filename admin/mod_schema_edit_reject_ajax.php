...
try {
    ...
    if (
        ...
        empty($_POST['schema_reason_rejected']) ||
    ...
    $schemaRejectedReason = htmlentities($_POST['schema_reason_rejected']);
    $schemaApproverUserID64 = $_SESSION['user_id64'];

    $schemaCheck = cached_query(
        'admin_schema_check_reject',
        'SELECT *
    ...
    $updateSQL = $db->q(
        'UPDATE `s2_mod_custom_schema`
            ...
            WHERE
                `modID` = ? AND
                `schemaApproved` <> 1 AND
                `schemaRejectedReason` IS NULL AND
                `schemaID` <> ?;
        ',
        'ssii',
        $schemaRejectedReason, $schemaApproverUserID64, $schemaModID, $schemaID
    );

    if ($updateSQL) {
        $json_response['result'] = "Success! Custom Game Schemas for mod #$schemaModID now rejected.";
        ...
        throw new Exception('No changes made to DB. There were probably no schemas to reject.');
    }
} catch (Exception $e) {
